...
- **WireGuard (tunnel already exists):** confirm the VPS-side peer's `AllowedIPs` covers `10.201.2.101/32` (or the `10.201.2.0/24` VLAN), and that the internal side has a return route for `10.201.4.9` (inter-VLAN routing already handles this). The VPS is the outbound-connecting peer, so `PersistentKeepalive = 25` belongs on its wg0 config to hold the path open. Runbook smoke test: `curl http://10.201.2.101:5400/login` from cyclorama, note the source IP in gunicorn's access log — that IP is the value for `TRUSTED_PROXY_IPS`/`GATEWAY_PEER_IPS` and the firewall rule.
- **VPS (cyclorama, firewalld):** allow http + https services only — the VPS is an outbound-connecting WG peer, so no WireGuard listen port is opened; fail2ban sshd jail + optional `321gateway` jail on `GATE_OTP_FAIL` (e.g. 10 fails/10 min ⇒ 1 h ban), with bans routed through firewalld rich rules (`banaction = firewallcmd-rich-rules`).
