...
def _send_pdf_email(show_id, pdf_type, triggered_by, exported_by_id=None, days_before=None):
    ...
    try:
        with app.app_context():
            if pdf_type == 'advance':
                _, pdf_version, show_dict, pdf_bytes, pdf_log_id, pdf_is_new = _build_advance_pdf(
                    show_id, exported_by_id=exported_by_id, base_url='/'
                )
            elif pdf_type == 'postnotes':
                _, pdf_version, show_dict, pdf_bytes, pdf_log_id, pdf_is_new = _build_postnotes_pdf(
                    show_id, exported_by_id=exported_by_id, base_url='/'
                )
            else:
                _, pdf_version, show_dict, pdf_bytes, pdf_log_id, pdf_is_new = _build_schedule_pdf(
                    show_id, exported_by_id=exported_by_id, base_url='/'
                )
    except Exception as e:
    ...
    # Skip when we reused an existing version — its bytes are already archived.
    if pdf_is_new and pdf_bytes and pdf_log_id and s3_storage.is_configured():
        try:
...
# Content-hash caching for generated documents.
#
# Every export used to bump the per-type version counter and write a brand-new
# export_log row, so repeatedly clicking "Export" (instead of re-downloading the
# already-generated file) produced a flood of near-identical versions. Instead
# we now fingerprint everything that lands in the rendered document and only
# cut a NEW version when that fingerprint changes; an unchanged document serves
# the previously generated bytes and keeps the same version number.
#
# The fingerprint is taken over the fully rendered HTML with the two genuinely
# volatile bits normalised out (the version label and the export timestamp), so
# it automatically tracks "all fields" that actually appear in the document —
# no per-field enumeration to keep in sync. Callers fold in anything that
# affects the PDF but not the HTML (e.g. appended file attachments) via `extra`.
# This path is shared by interactive exports AND the background/scheduled email
# jobs, so both agree on when a new version is warranted.

# Volatile template values substituted in while rendering the copy we hash, so
# the timestamp and version label never perturb the fingerprint.
_HASH_VERSION_SENTINEL = 0
_HASH_DATE_SENTINEL = '__EXPORT_DATE__'


def _pdf_content_hash(html, extra=''):
    """SHA-256 fingerprint of a rendered document's content."""
    h = hashlib.sha256()
    h.update((html or '').encode('utf-8', 'replace'))
    if extra:
        h.update(b'\x00')
...
def _load_export_bytes(row):
    """Return the stored PDF bytes for an export_log row (S3 or inline blob),
...
def _find_reusable_export(db, show_id, export_type, content_hash):
    """If the most recent export of this type has the same content fingerprint
    and its bytes are still retrievable, return (row, pdf_bytes) so the caller
    can serve it unchanged. Otherwise return (None, None) → build a new version.

    Only the latest export is considered: a change always cuts a new version,
    and reverting a change likewise cuts a new version rather than resurrecting
...
def _stash_export_bytes(log_id, pdf_bytes):
    """Persist freshly built PDF bytes onto the export_log row immediately.

    The interactive routes push to S3 in a background thread and only THEN set
    s3_key, so without this a rapid second click (a common cause of version
...
def _advance_attachments_fingerprint(db, show_id):
    """Stable fingerprint of the files that get appended to an advance PDF.

    Covers attachment identity and size so that adding, removing, or replacing
    a file changes the advance content hash (and thus cuts a new version) even
...
def _build_advance_pdf(show_id, exported_by_id=None, base_url=None):
    ...
    attach_fp = _advance_attachments_fingerprint(db, show_id)
    ...
    def _render(version, export_date):
        try:
            return render_template('pdf/advance_pdf.html',
    ...
    # Reuse the previous version's bytes when nothing that lands in the document
    # has changed, so repeat clicks don't spawn redundant versions.
    content_hash = _pdf_content_hash(
        _render(_HASH_VERSION_SENTINEL, _HASH_DATE_SENTINEL), extra=attach_fp)
    db = get_db()
    cached_row, cached_bytes = _find_reusable_export(db, show_id, 'advance', content_hash)
    if cached_row:
        db.close()
        syslog_logger.info(
            f"PDF_EXPORT_REUSED show_id={show_id} type=advance v={cached_row['version']} by={exported_by_id}"
        )
    ...
    _stash_export_bytes(log_id, pdf_bytes)
    syslog_logger.info(
...
def _build_schedule_pdf(show_id, exported_by_id=None, base_url=None):
    ...
    if cached_row:
        db.close()
        syslog_logger.info(
            f"PDF_EXPORT_REUSED show_id={show_id} type=schedule v={cached_row['version']} by={exported_by_id}"
        )
        return None, cached_row['version'], dict(show), cached_bytes, cached_row['id'], False
...
def export_advance(show_id):
    if not can_access_show(session['user_id'], show_id):
        abort(403)
    get_show_or_404(show_id)
    html, version, show, pdf_bytes, log_id, is_new = _build_advance_pdf(show_id)
    ...
    if pdf_bytes:
        ...
        # Only for freshly built versions — a reused version is already archived.
        if is_new and s3_storage.is_configured():
...
def export_schedule(show_id):
    if not can_access_show(session['user_id'], show_id):
        abort(403)
    get_show_or_404(show_id)
    html, version, show, pdf_bytes, log_id, is_new = _build_schedule_pdf(show_id)
    safe_name = show['name'].replace(' ', '_').replace('/', '-')
...
def _build_postnotes_pdf(show_id, exported_by_id=None, base_url=None):
    ...
    # Reuse the previous version's bytes when nothing in the document changed.
    content_hash = _pdf_content_hash(_render(_HASH_VERSION_SENTINEL, _HASH_DATE_SENTINEL))
    cached_row, cached_bytes = _find_reusable_export(db, show_id, 'postnotes', content_hash)
    if cached_row:
        db.close()
        syslog_logger.info(
            f"PDF_EXPORT_REUSED show_id={show_id} type=postnotes v={cached_row['version']} by={exported_by_id}"
        )
        return None, cached_row['version'], dict(show), cached_bytes, cached_row['id'], False
...
def export_postnotes(show_id):
    if not can_access_show(session['user_id'], show_id):
        abort(403)
    get_show_or_404(show_id)
    html, version, show_dict, pdf_bytes, _log_id, _is_new = _build_postnotes_pdf(show_id)
